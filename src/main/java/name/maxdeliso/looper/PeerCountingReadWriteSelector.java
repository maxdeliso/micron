...
public interface PeerCountingReadWriteSelector {
    ...
    default Optional<Peer> lookupPeer(final SelectionKey selectionKey, final PeerRegistry peerRegistry) {
        return ofNullable(selectionKey).map(key -> (Long) key.attachment()).flatMap(peerRegistry::get);
    }

    default void handleReadableKey(final SelectionKey readSelectedKey,
                                   final PeerRegistry peerRegistry,
                                   final Consumer<Peer> peerConsumer) {
        lookupPeer(readSelectedKey, peerRegistry).ifPresent(peerConsumer);
    }

    default void handleWritableKey(final SelectionKey writeSelectedKey,
                                   final PeerRegistry peerRegistry,
                                   final Consumer<Peer> peerConsumer) {
